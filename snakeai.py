...
def get_state():
    ...
    # Direction to food
    food_dx = np.sign(fx - x)
    food_dy = np.sign(fy - y)

    # Wall proximity
    wall_up = (x == 0)
    wall_down = (x == GRID_SIZE - 1)
    wall_left = (y == 0)
    wall_right = (y == GRID_SIZE - 1)

    # Obstacles (snake body)
    obstacle_up    = (x-1, y) in snake.body[1:]
    obstacle_down  = (x+1, y) in snake.body[1:]
    obstacle_left  = (x, y-1) in snake.body[1:]
    obstacle_right = (x, y+1) in snake.body[1:]
...
while True:
    ...
    pygame.draw.rect(screen, RED,
                     (fx*BLOCK_SIZE, fy*BLOCK_SIZE, BLOCK_SIZE-1, BLOCK_SIZE-1))
